...
class CodeAgent:
    ...
    def __init__(self) -> None:
        ...
        self._model = os.getenv("LITELLM_MODEL", "gemini/gemini-2.0-flash")
    ...
    async def run(
        ...
        *wait_for_command* is an async generator that yields the next
        ...
        if self._llm_settings.get("model"):
            ...
            # Re-evaluate backend based on user-provided model
            self._backend = _get_llm_backend(self._model)
            logger.info("Using user-provided model: %s (backend: %s)", self._model, self._backend)
